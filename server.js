...
const { passport, ensureAuthenticated, requireRole, ensureNotReadOnly, ROLES } = require('./lib/auth-config');
...
// IMPORTANT: Uses Redis (same ElastiCache cluster as REDIS_URL below) so that
// sessions created here are readable by the Web EC2 tier, and vice versa.
// Without this, Web EC2 cannot validate a session cookie issued by App EC2.
app.use(cookieParser());

if (process.env.NODE_ENV === 'production' && !process.env.REDIS_URL) {
    console.error('❌ CRITICAL: REDIS_URL is required in production for shared sessions across Web/App EC2 tiers');
    process.exit(1);
}

const sessionRedisClient = process.env.REDIS_URL ? new Redis(process.env.REDIS_URL) : null;
if (sessionRedisClient) {
    sessionRedisClient.on('error', (err) => console.error('⚠️ [SESSION-REDIS] Error:', err.message));
}

app.use(session({
    store: sessionRedisClient ? new RedisStore({ client: sessionRedisClient, prefix: 'acc-sess:' }) : undefined, // falls back to MemoryStore only in local dev
...
// NOTE: Static frontend files (public/) and page routes (/, /login, /compliance,
// /siem-docs) are now served by the Web EC2 tier (web-server/). This server is
// API-only — ALB routes /api/*, /auth/*, and /socket.io/* here.
...
// Login page is served by the Web EC2 tier. ALB routes everything except
// /api/*, /auth/*, /socket.io/* to Web EC2 — this server only handles the
...
// Page routes (/, /siem-docs, /compliance) are served by the Web EC2 tier,
// which runs its own ensureAuthenticated/requireRole guards against the
// same shared Redis session store. See web-server/server.js.
...
// (Duplicate /compliance page route removed — page routes now live in web-server/)
